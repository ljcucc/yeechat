...
class MainApp extends LitElement {
  ...
  static properties = {
    _bufferList: {type: Object},
    _bufferPointers: {type: Object},
    _dispLogs: {type: Array},
    _title: {type: String},
    _selectedBuffer: {type: String},
  ...
  constructor(){
    ...
    this.socket.on("init", e=>{
      ...
      .then(a=>{
        ...
        this._bufferList = Object.keys(e).reduce((acc,cur)=>{acc[e[cur]]=cur;return acc}, {});
      });
    ...
    this.socket.on("new:msg", e=>{
      console.log(e);
      if(e.buffer == this._selectedBuffer){
  ...
  onChatSelected(e) {
    console.log(this._bufferList);
    this._title = this._bufferPointers[e.detail.id];
    this._selectedBuffer = e.detail.id;

    const bufferid = e.detail.id,
    sid = this.socket.id;
  ...
  onMsgSend(e){
    console.log(e)
    this.socket.emit('message', this._selectedBuffer, e.detail.value);
  }
  ...
  render() {
    ...
    <div class="app">
      <div class="layout">
        <buffer-list .list=${this._bufferList} @select="${this.onChatSelected}" class="${classMap({close:this._showlist })}"></buffer-list>
        <chat-box 
    ...
    ${this._openSettings?
    html`<settings-page .options=${{
      irc_url: "chat.ljcu.cc",
      irc_port: 9001,
      password: ""
    }} @close="${this.settingsClose}"></settings-page>`
    : ""}
    `;
  }
}
